...
const DownSpeedTop = topCom('下载速率TOP5', '下载速率', 'downSpeedTop', num => unitConvert(num * 1024))
const UpSpeedTop = topCom('上传速率TOP5', '上传速率', 'upSpeedTop', num => unitConvert(num * 1024))
...
const reducer = createReducer({
  [newDataAct]: (state, evt) => {
    function getTopData(index) {
      const as = evt.map(row => ({ ip: row[0], count: row[index], mac: row[8], hostName: row[9] })).sort((a, b) => b.count - a.count)
      return {
        tops: as.splice(0, 5),
    ...
    if (findSelf) {
      ...
      selfInfo.downSpeed = findSelf[4]
      selfInfo.upSpeed = findSelf[3]
      ...
      selfInfo.hostName = findSelf[9]
    }

    selfInfo.totalDownload = getTotal(2)
    selfInfo.totalDownSpeed = getTotal(4)
    selfInfo.totalUpSpeed = getTotal(3)
